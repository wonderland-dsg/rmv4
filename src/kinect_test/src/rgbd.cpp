...
private:
  void start(const Mode mode)
  {
    ...
    imagePub = it.advertise("myImage", queueSize);
    depthPub = it.advertise("myDepth", queueSize);
  ...
  {
    imagePub.publish(imageColor);
    depthPub.publish(imageDepth);
    
    OUT_INFO("republish count: " << ++myCount);
    ...
    for(; running && ros::ok();)
    {
      if(updateImage)
      {
        ...
        if(elapsed >= 1.0)
        {
          ...
          OUT_INFO("fps: " << fps << " ( " << elapsed / frameCount * 1000.0 << " ms)");
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "rgbd", ros::init_options::AnonymousName);
  ...
  Receiver::Mode mode = Receiver::NO;
